...
export const MessageBox2 = (props) => {
  ...
  >
    ...
    >
      <header style={{backgroundColor:'var(--black)',color:'white'}} 
      className="w3-container"><h3>{title?title:'Message'}</h3></header>
      <div className="w3-container"
        {...dprops}
      >{component}</div>
...
export const MessageBox3 = (props) => {
  const [status, setStatus] = useState(null);
  let { close, message,dispatch ,state, change, display, buttons, component, size } = props;
  //let{title}=props;
    ...
    >
      ...
      >
        <div className="w3-container w3-margin" >
          <a className="w-btn w3-large w3-blue" href="/static/signin.html">Login</a>
        </div>
        <hr />
        <div className="w3-container w3-margin" >
          <button onClick={turnOff} className="w3-btn w3-large w3-blue">Continue without Logging in</button>
        </div>
      </div>
      {/*<footer className="w3-bar w3-center w3-container">
